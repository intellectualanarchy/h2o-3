    ...
    def _jar_paths():
        ...
        if own_jar != "":
            if not os.path.isfile(own_jar):
                raise H2OValueError("Environment variable H2O_JAR_PATH is set to '%d' but file does not exists, unset environment variable or provide valid path to h2o.jar file." % own_jar)
            return own_jar
